...
if not images:
    logger.error(f"Не найдено изображений в папке {PAINTINGS_DIR}")
    exit()

logger.info(f"Найдено изображений: {len(images)}")
image = cv2.imread(os.path.join(PAINTINGS_DIR, images[0]))
logger.info(f"Обработка изображения: {images[0]}")
...
logger.info(f"полутоновый ручной: {time.time() - start:.4f} сек")

start = time.time()
gray_cv = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
logger.info(f"полутоновый opencv: {time.time() - start:.4f} сек")
...
logger.info(f"свертка ручная: {time.time() - start:.4f} сек")

start = time.time()
conv_cv = cv2.filter2D(gray_cv_base, -1, kernel)
logger.info(f"свертка opencv: {time.time() - start:.4f} сек")

# gaussian
start = time.time()
gaussian_manual = smooth_manual(gray_cv_base, 5)
logger.info(f"гаусс ручной: {time.time() - start:.4f} сек")

start = time.time()
gaussian_cv = cv2.GaussianBlur(gray_cv_base, (5, 5), 0)
logger.info(f"гаусс opencv: {time.time() - start:.4f} сек")


# sobel
start = time.time()
sobel_manual = detect_edges_manual(gray_cv_base)
logger.info(f"собель ручной: {time.time() - start:.4f} сек")
...
logger.info(f"собель opencv: {time.time() - start:.4f} сек")

# gamma
start = time.time()
gamma_manual = gamma_correction_manual(image, 2.2)
logger.info(f"гамма ручной: {time.time() - start:.4f} сек")

start = time.time()
inv_gamma = 1.0 / 2.2
table = ((np.arange(256) / 255.0) ** inv_gamma * 255).astype(uint8)
gamma_cv = cv2.LUT(image, table)
logger.info(f"гамма opencv: {time.time() - start:.4f} сек")
...
logger.info("Обработка завершена, результаты сохранены в папке paintings")
